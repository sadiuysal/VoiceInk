...
class NativeAppleTranscriptionService: TranscriptionService {
	private let logger = Logger(subsystem: "com.sadiuysal.VoiceInk", category: "NativeAppleTranscriptionService")
	...
	func transcribe(audioURL: URL, model: any TranscriptionModel) async throws -> String {
		...
		guard #available(macOS 26, *), canUseNativeSpeechAnalyzer else {
			logger.error("SpeechAnalyzer is not available on this macOS version or build configuration")
	...
	// Build-time helper: true when we compile with ENABLE_NATIVE_SPEECH_ANALYZER
	private var canUseNativeSpeechAnalyzer: Bool {
