...
actor WhisperContext {
	...
	private let logger = Logger(subsystem: "com.sadiuysal.VoiceInk", category: "WhisperContext")
...
// Stub fallback when the whisper module is not available.
actor WhisperContext {
	private let logger = Logger(subsystem: "com.sadiuysal.VoiceInk", category: "WhisperContext")
	private init() {}
	init(context: OpaquePointer) {}
	...
	static func createContext(path: String) async throws -> WhisperContext {
		let logger = Logger(subsystem: "com.sadiuysal.VoiceInk", category: "WhisperContext")
		logger.error("whisper module not available; cannot create context")
		throw WhisperStateError.modelLoadFailed
	}
	
	func fullTranscribe(samples: [Float]) -> Bool {
		let logger = Logger(subsystem: "com.sadiuysal.VoiceInk", category: "WhisperContext")
		logger.error("whisper module not available; returning failure from fullTranscribe")
		return false
	}
